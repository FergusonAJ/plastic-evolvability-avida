...
def write_mutant_spop_file(filename, org, inst_set, header = '', cols_to_write = None):
    cur_id = 0
    ...
    with open(filename, 'w') as fp:
        ...
        for locus_idx in range(len(org['sequence'])):
            ...
            for inst_idx in range(len(inst_set)):
                inst_char = inst_set[inst_idx][1]
                if inst_char == base_char: # Don't duplicate the base org
                    continue
                sequence = seq_prefix + inst_char + seq_suffix
...
if __name__ == '__main__':
    ...
    write_mutant_spop_file(output_filename, dom_org, inst_set, header_str, cols_to_write)
